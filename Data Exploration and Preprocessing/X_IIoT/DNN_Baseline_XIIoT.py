...
Y_train = F.one_hot(Y_train).float()
Y_val = F.one_hot(Y_val).float()
Y_test = F.one_hot(Y_test).float()
...
model = SimpleMLP(input_size=56, num_classes=19)
#%%
...
#%%
torch.save(model, './models/DNN_X_IIoT.pth')
#%%
# load the model
model = torch.load('./models/DNN_X_IIoT.pth')
