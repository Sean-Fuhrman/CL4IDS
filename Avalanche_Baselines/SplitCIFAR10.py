...
def test_strategy(name, epochs=1):
    ...
    if name == "ICaRL": #Replay methods
        feature_size = 64 
        feature_extractor = SimpleMLP(input_size=3072, num_classes=feature_size)
        ...
        model = MTSimpleMLP(input_size=3072)
        ...
        model = SimpleMLP(input_size=3072, num_classes=benchmark.n_classes) #needs very high prune proportion to work
...
if __name__ == "__main__":
    # strategies = ["EBLL", "EWC", "SI", "MAS", "mean-IMM", "mode-IMM", "PackNet", "HAT", "Cumulative", "Naive"]
    # for strategy in strategies:
    #     test_strategy(strategy)
    test_strategy("Naive-Multihead", 1)
